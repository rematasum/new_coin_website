...
async function deploy() {
  ...
  const instantBps  = [0n, 0n, 0n, 0n, 0n, 0n]; // all 100% monthly vesting
...
describe("TeamVesting", () => {
  describe("deployment", () => {
    it("stores correct beneficiary allocations", async () => {
      ...
      expect(teamData.instantUnlockBps).to.equal(0n);
    });
  ...
  describe("claim", () => {
    ...
    it("reverts before vestingStart (nothing to claim — 0% instant)", async () => {
      const { tv, team } = await deployAndStart();
      await expect(tv.connect(team).claim()).to.be.revertedWith("Nothing to claim");
    });

    it("after 1 month: 1/24 of allocation claimable", async () => {
      ...
      const balance   = await token.balanceOf(team.address);
      const expected  = TEAM_AMOUNT / 24n;
      expect(balance).to.be.closeTo(expected, hre.ethers.parseEther("1"));
    });

    it("after 12 months: 12/24 of allocation claimable", async () => {
      ...
      const expected = (SPONSOR_AMOUNT * 12n) / 24n;
    ...
    it("second claim only gets newly vested tokens", async () => {
      ...
      expect(after1).to.be.closeTo(TEAM_AMOUNT / 24n, hre.ethers.parseEther("1"));
  ...
  describe("getClaimableNow", () => {
    ...
    it("returns correct amount after 1 month", async () => {
      ...
      expect(claimable).to.be.closeTo(TEAM_AMOUNT / 24n, hre.ethers.parseEther("1"));
    });
  });
